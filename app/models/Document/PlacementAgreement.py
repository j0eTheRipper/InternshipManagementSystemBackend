...
class PlacementAgreement(Document):
    placement_agreement_id: int

    _table: ClassVar[str] = "placement_agreement"
    _id_field: ClassVar[str] = "placement_agreement_id"
    _upload_dir: ClassVar[str] = "uploads/placement_agreements"
    _allowed_extensions: ClassVar[set[str]] = {"pdf"}
    _columns: ClassVar[list[str]] = ["placement_agreement_id", "student_id", "file", "verified"]

    @staticmethod
    def save_in_db(student_id: str, file_path: str) -> int:
        connection = database_connector.create_connection(False)
        query = f"INSERT INTO placement_agreement (student_id, file) VALUES ('{student_id}', '{file_path}');"
        database_connector.execute_write(connection, query)
        result = database_connector.execute_read(
            connection,
            f"SELECT MAX(placement_agreement_id) FROM placement_agreement WHERE student_id = '{student_id}';",
        )
